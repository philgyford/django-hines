...
class BlogYearsTestCase(TestCase):
    def setUp(self):
        ...
        LivePostFactory(
            blog=self.blog, time_published=make_datetime("2016-03-01 12:00:00")
        )
        LivePostFactory(
            blog=self.blog, time_published=make_datetime("2017-03-01 12:00:00")
        )

        # This is a draft, so 2015 shouldn't be included:
        DraftPostFactory(
            blog=self.blog, time_published=make_datetime("2015-03-01 12:00:00")
        )

        # This is from a different blog, so 2014 shouldn't be included.
        LivePostFactory(time_published=make_datetime("2014-03-01 12:00:00"))
