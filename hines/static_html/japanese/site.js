...
(function(){
    ...
    function initListeners() {
      ...
      getElByClass('js-restart-button').addEventListener('click', handleRestartConfirm);
    ...
    function storeProgressInStorage() {
      if (storageAvailable('localStorage')) {
        ...
        localStorage.setItem('jpUsedCharacters', JSON.stringify(usedCharacters));
      };
    };
    ...
    function showNewLetter() {
      ...
      if (characterPool.length - usedCharacters.length === 1) {
        ...
        while(usedCharacters.indexOf(letter) > -1 || letter === '' || letter === currentLetter) {
          letter = characterPool[Math.floor(Math.random() * characterPool.length)];
        };
      };
