...
class Command(BaseCommand):
    ...
    def import_publication_series(self):
        ...
        try:
            with self.connection.cursor() as cursor:
                sql = (
                    "SELECT * FROM `phil_publicationseries` "
                ...
                for series in cursor:
                    ...
                    new_series, created = PublicationSeries.objects.update_or_create(
                        pk=series["id"], defaults=defaults
                    )
                    series_count += 1
    ...
    def import_publications(self):
        ...
        try:
            with self.connection.cursor() as cursor:
                sql = "SELECT * FROM `phil_publication` " "ORDER BY `id` ASC {}".format(
                    LIMIT
                )
                cursor.execute(sql, ())
                ...
                for pub in cursor:
                    ...
                    if pub["publicationseries_id"]:
                        try:
                            series = PublicationSeries.objects.get(
                                pk=pub["publicationseries_id"]
                            )
                            ...
                            print(
                                "No PublicationSeries found with ID '{}'".format(
                                    pub["publicationseries_id"]
                                )
                            )

                    new_pub, created = Publication.objects.update_or_create(
                        pk=pub["id"], defaults=defaults
                    )
    ...
    def import_authors(self):
        ...
        try:
            with self.connection.cursor() as cursor:
                sql = "SELECT * FROM `phil_author` " "ORDER BY `id` ASC {}".format(
                    LIMIT
                )
                cursor.execute(sql, ())
                ...
                for author in cursor:
                    ...
                    defaults = {"name": " ".join(names)}

                    new_author, created = Creator.objects.update_or_create(
                        pk=author["id"], defaults=defaults
                    )
                    author_count += 1
        except Exception as e:
            raise CommandError(e)
    ...
    def import_author_publications(self):
        ...
        try:
            with self.connection.cursor() as cursor:
                sql = (
                    "SELECT * FROM `phil_author_publication` "
                    "ORDER BY `id` ASC {}".format(LIMIT)
                )
                cursor.execute(sql, ())
                ...
                for ap in cursor:
                    try:
                        ...
                        print(
                            "No Publication found with ID '{}'".format(
                                ap["publication_id"]
                            )
                        )
                        continue
                    ...
                    PublicationRole.objects.update_or_create(
                        creator=creator, publication=publication, role_name=role_name
                    )
                    ap_count += 1
        except Exception as e:
            raise CommandError(e)
        ...
        print(
            "Imported {} connection(s) between authors and publications".format(
                ap_count
            )
        )
    ...
    def import_readings(self):
        ...
        try:
            with self.connection.cursor() as cursor:
                sql = "SELECT * FROM `phil_reading` " "ORDER BY `id` ASC {}".format(
                    LIMIT
                )
                cursor.execute(sql, ())
                ...
                for reading in cursor:
                    try:
                        publication = Publication.objects.get(
                            pk=reading["publication_id"]
                        )
                    except Publication.DoesNotExist:
                        print(
                            "No Publication found with ID '{}'".format(
                                reading["publication_id"]
                            )
                        )
                        continue
                    ...
                    if start_date is not None:
                        if re.match(r"\d\d\d\d-00-00", start_date):
                            start_granularity = 6
                        elif re.match(r"\d\d\d\d-\d\d-00", start_date):
                            start_granularity = 4

                        start_date = start_date.replace("-00", "-01")
                        start_date = datetime.strptime(start_date, "%Y-%m-%d").date()
                    if end_date is not None:
                        if re.match(r"\d\d\d\d-00-00", end_date):
                            end_granularity = 6
                        elif re.match(r"\d\d\d\d-\d\d-00", end_date):
                            end_granularity = 4
